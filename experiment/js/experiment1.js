...
var stimuli = [
  ...
  'img/frac21.png', 'img/frac22.png',
];
stimuli = jsPsych.randomization.repeat(stimuli,1,false);

var stimuli_trials_1 = [
  { stimuli_trials: stimuli[0], data_trials:{state: 1, test_part:'trials', correct_response:49}},
  { stimuli_trials: stimuli[1], data_trials:{state: 2, test_part:'trials', correct_response:50}},
  { stimuli_trials: stimuli[2], data_trials:{state: 3, test_part:'trials', correct_response:51}},
  { stimuli_trials: stimuli[3], data_trials:{state: 4, test_part:'trials', correct_response:52}},
];
var stimuli_trials_2 = [
  { stimuli_trials: stimuli[4], data_trials:{state: 1, test_part:'trials', correct_response:49}},
  { stimuli_trials: stimuli[5], data_trials:{state: 2, test_part:'trials', correct_response:50}},
  { stimuli_trials: stimuli[6], data_trials:{state: 3, test_part:'trials', correct_response:51}},
  { stimuli_trials: stimuli[7], data_trials:{state: 4, test_part:'trials', correct_response:52}},
];
var stimuli_trials_3 = [
  { stimuli_trials: stimuli[8], data_trials:{state: 1, test_part:'trials', correct_response:49}},
  { stimuli_trials: stimuli[9], data_trials:{state: 2, test_part:'trials', correct_response:50}},
  { stimuli_trials: stimuli[10], data_trials:{state: 3, test_part:'trials', correct_response:51}},
  { stimuli_trials: stimuli[11], data_trials:{state: 4, test_part:'trials', correct_response:52}},
];


// Randomly generate stimulus sets for each iti. 
const multiplyArray = (arr, length) =>  Array.from({ length }, () => arr).flat()
stimuli_trials_1 = multiplyArray(jsPsych.randomization.repeat(stimuli_trials_1,1,false),   2);
stimuli_trials_2 = multiplyArray(jsPsych.randomization.repeat(stimuli_trials_2,1,false),   2);
stimuli_trials_3 = multiplyArray(jsPsych.randomization.repeat(stimuli_trials_3,1,false),   2);
var stimuli_trials_agg = {'0': stimuli_trials_1, '500': stimuli_trials_2, '2000': stimuli_trials_3};

// Instantiate variables
  var superblock_order = jsPsych.randomization.repeat([0,500,2000],1,false); // ITI: 0, 500, 2000 milliseconds
  var iti = [0,500,2000].concat(superblock_order[0]).concat([0,500,2000]).concat(superblock_order[1]).concat([0,500,2000]).concat(superblock_order[2])
  ...
  //var block_durations = multiplyArray([3,3,3,6], num_superblocks).map(function(element) {return element * 1000;});
  ...
  for (j=0; j<num_superblocks; j++) {
    ...
    (function(superblock_temp,j_temp) {
      ...
      function timerDisplay() {
        ...
        currentblock_iti = superblock_temp[j_temp];
        num_blocks_thistask = 4;

        stimuli_trials = stimuli_trials_agg[currentblock_iti];
        key_choices = ['1','2','3','4'];
        keys_html = '<b>"1","2","3", or "4",</b>';
        ...
        if(j_temp==0){
          var instructions_superblock = {
              ...
              '<p class="center-content">In this study, you will earn <b style="color:#32CD32">$4</b> plus a bonus of <b style="color:#32CD32">$1-2</b> depending on your performance!</b></p>' +
              '<p class="center-content">The whole study should take you about <b>25 minutes</b> in total, which includes a short survey at the end of the experiment. </p>' +
              ...
              '<p> <b>Note: Different images may share the same "most-likely-rewarding" key! </b></p>' +
              '<p> &nbsp;</p>'+
              ...
              '<p> &nbsp; </p>' +
              '<p><i>The relationship between number keys and rewards for each image will <b>stay the same</b> within this task.</i></p>'+
    ...
    for (i = 0; i < num_blocks_thistask; i++) {
        // Immediately-Invoked Function Expression (IIFE)
        (function(superblock_temp,j_temp) {
          var current_iti = currentblock_iti;
...
} // loop through superblocks with different ITIs.
...
var end_experiment = {
  ...
  on_finish: function(data) {
    window.location.href = "https://harvard.az1.qualtrics.com/jfe/form/SV_a4QUBIHPUMrff9Q?&workerId=" + turkInfo.workerId + "&assignmentId=" + turkInfo.assignmentId + "&hitId=" + turkInfo.hitId;
  },
};
